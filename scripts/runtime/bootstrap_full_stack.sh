...
DATABASE_URL="$(resolve_runtime_route_value "$ROOT_DIR" "DATABASE_URL" "" "postgresql+psycopg://postgres:postgres@127.0.0.1:5432/sourceharbor")"
export DATABASE_URL
...
if command -v psql >/dev/null 2>&1; then
  DB_URL="${DATABASE_URL:-postgresql+psycopg://postgres:postgres@127.0.0.1:5432/sourceharbor}"
  PSQL_URL="${DB_URL/postgresql+psycopg:\/\//postgresql://}"
  if [[ "$DB_URL" == postgresql* ]]; then
    DB_NAME="$(python3 - <<'PY'
import os
from urllib.parse import urlparse
u = os.getenv('DATABASE_URL', 'postgresql+psycopg://localhost:5432/sourceharbor')
...
import json, os
from urllib.parse import urlparse
u = os.getenv('PSQL_URL','postgresql://localhost:5432/sourceharbor')
p = urlparse(u)
print(json.dumps({
